...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    while(queue.length) {
      ...
      let shouldRessignClosest = currentVal < closest || closest === null;

      if(higherThanLowerBound && shouldRessignClosest) {
        closest = currentVal;
      }
  ...
  areCousins(node1, node2) {
    ...
    let diffrentParents = node1Info && node2Info && node1Info.parent !== node2Info.parent;
    return sameLevel && diffrentParents;
  }
  ...
  static serialize(tree) {
    ...
    //pre order
    // function serializeHelper(str, node) {
    //   if(!node) {
    //     return str += 'null,';
    //   }
    //   str = str + node.val + ',';
    //   str = serializeHelper(str, node.left);
    //   str = serializeHelper(str, node.right);
    //   return str;
    // }
  ...
  static deserialize(stringTree) {
    ...
    function buildTree() {
      // buildin a tree starting from the beginning of the array
      if(values.length) {
    ...
    // function deserializeHelper(list) {
    //   if(list[0] === 'null') {
    //     list.shift();
    //     return null;
    //   }
    //   let node = new BinaryTree(list[0]);
    //   list.shift();
  ...
  lowestCommonAncestor(node1, node2, currentNode=this.root) {
    ...
    //recursively search the left and right sub-trees
